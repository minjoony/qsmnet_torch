...
class test_dataset():
    def __init__(self, args):
        try:
            data_file = scipy.io.loadmat(args.TEST_PATH + args.TEST_FILE)
        except:
            data_file = mat73.loadmat(args.TEST_PATH + args.TEST_FILE)
            
        value_file = scipy.io.loadmat(args.VALUE_FILE_PATH + args.VALUE_FILE_NAME)
        cosmos_value_file = scipy.io.loadmat(args.PRE_NET_CHECKPOINT_PATH + 'qsm_train_patch_norm_factor.mat')
        ...
        self.field = crop_img_16x(field_in_ppm)
        self.mask = crop_img_16x(data_file['mask_4d'])
                
        if args.LABEL_EXIST is True:
            self.qsm = data_file['cosmos_4d']
            
        if args.CSF_MASK_EXIST is True:
            self.csf_mask = data_file['csf_mask_4d']
